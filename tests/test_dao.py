...
class Test_import(unittest.TestCase):
    ...
    def test_import_t(self):
        ...
        table = "demo_teachers"
        db = Database("demo.db")
        db.cur.execute("DROP TABLE IF EXISTS " + table)
        db.cur.execute('''CREATE TABLE "demo_teachers" (
            "teacher_id" VARCHAR(50) NOT NULL,
            "teacher_name" VARCHAR(50) NOT NULL,
            PRIMARY KEY ("teacher_id"));''')
        db.import_t(csvfile, table)
    ...
    def test_import_a(self):
        ...
        table = "demo_assignment"
        db = Database("demo.db")
        db.cur.execute("DROP TABLE IF EXISTS " + table)
        db.cur.execute('''CREATE TABLE "demo_assignment" (
